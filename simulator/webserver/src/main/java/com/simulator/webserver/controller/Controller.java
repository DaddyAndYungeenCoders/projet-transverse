...
public class Controller {
    ...
    public String updateUser(@PathVariable Long id, @RequestBody UserEntity updatedUser) {
        UserEntity result = dbService.updateUser(id, updatedUser.getUsername(), updatedUser.getPassword());

        if (result != null) {
            return "Utilisateur avec l'ID " + id + " mis à jour avec succès.";
        } else {
            return "La mise à jour de l'utilisateur avec l'ID " + id + " a échoué. Utilisateur non trouvé.";
        }
    }
